...
#include "spraybot_lifecycle_manager/lifecycle_manager.hpp"
#include "spraybot_lifecycle_manager/lifecycle_manager_client.hpp"
...
TEST(LifecycleBondTest, POSITIVE)
{
  ...
  spraybot_lifecycle_manager::LifecycleManagerClient client("lifecycle_manager_test", node);
  ...
  rclcpp::Rate(5).sleep();
  EXPECT_EQ(
    spraybot_lifecycle_manager::SystemStatus::INACTIVE,
  ...
  EXPECT_TRUE(bond_tester->isBondConnected());
  EXPECT_EQ(
    spraybot_lifecycle_manager::SystemStatus::ACTIVE,
    client.is_active(std::chrono::nanoseconds(1000000000)));
